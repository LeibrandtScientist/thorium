...
class SerialServer(LabradServer):
    ...
    def initServer(self):
        ...
        for name, _description, _hardware in ports:
    ...
    def timeout(self, c, data=T.Value(0, "s")):  # noqa: B008
    ...
    @setting(
        40, "Write", data=["s: Data to send", "*w: Byte-data to send"], returns=["w: Bytes sent"]
    )
    ...
    def read(self, c, count=0):
        """Reads data from the port as a unicode string."""
    ...
    def read_bytes(self, c, count=0):
        """Reads data from the port as bytes, and returns a list of ints representing the bytes."""
